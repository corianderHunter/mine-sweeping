...
Game.prototype.init = function () {
  ...
  grid_data.forEach(function (isBomb, i) {
    ...
    if (!isBomb) {
      var neighbors = neighbors_cords.map(function (xy) {
        return grid_data[getIndex(xy[0], xy[1])]
      })
      mine.mine_count = neighbors.filter(function (neighbor_bomb) {
        return neighbor_bomb
      }).length
    }
    mine.classList.add('x' + x, 'y' + y)
    mine.neighbors = neighbors_cords.map(function (xy) {
...
Game.prototype.game = function () {
  ...
  if (bombs.length > 0) {
    Array.prototype.forEach.call(masked, function (cell) {
      cell.reveal()
    })
    this.result = 'lost'
    this.showMessage()
